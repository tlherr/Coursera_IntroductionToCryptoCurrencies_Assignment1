 ...
 *
 * Based on the transaction it has chosen to accept, handleTxs should also update its internal UTXOPool to reflect the current
...
public class TxHandler {
    ...
    public boolean isValidTx(Transaction tx) {
        ...
        System.out.println(String.format("Transaction Valid: %b", (outputSum <= inputSum)));
    ...
    public Transaction[] handleTxs(Transaction[] possibleTxs) {
        ...
        Transaction[] tmpAcceptedTransactions = new Transaction[possibleTxs.length];

        UTXOPool currentPool = this.getCurrentPool();
        ...
        for(int i = 0; i < possibleTxs.length; i++)
            ...
            if(isValidTx(possibleTxs[i])) {
                //Add to an array of "accepted" transactions
                tmpAcceptedTransactions[validindex] = possibleTxs[i];
                validindex++;

                //Have to find the transaction in the UTXOPool and remove it
                for (UTXO currentUTXO: currentPool.getAllUTXO()) {
                    if(currentUTXO.getTxHash() == possibleTxs[i].getHash()) {
                        currentPool.removeUTXO(currentUTXO);
                    }
                }
            }

        return Arrays.copyOf(tmpAcceptedTransactions, validindex);
    }
